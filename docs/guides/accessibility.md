...
背景と前景の間の色のコントラストにも注意してください。例えば、脚注に薄いグレーの文字を使うと、読みにくくなることがあります。低コントラストを招きやすいもう一つの状況として、画像の上に文字を重ねる場合があります。
